...
JAVASCRIPT;
    ...
    public static function constructEventsArray($options = [])
    {
       ...
       // construct events (in fullcalendar format)
        ...
        foreach ($raw_events as $event) {
            ...
            // INFOTEL
            $content = Planning::displayPlanningItem($event, $users_id, 'in', false,$param) ?: ($event['content'] ?? "");
            $tooltip = Planning::displayPlanningItem($event, $users_id, 'in', true,$param) ?: ($event['tooltip'] ?? "");
            // INFOTEL
           ...
           // maybe we need a better way for displaying categories color
            ...
            if ($param['view_name'] == "resourceTimeGridDay") {
                ...
                if (!empty ($event['text_cat_color'])) {
                    $new_event['textColor'] = $event['text_cat_color'];
                }
            }
            //INFOTEL
     ...
     * @param $options array //INFOTEL
     *
     * @return string
     **/
    public static function displayPlanningItem(array $val, $who, $type = "", $complete = 0, $options = [])
    {
        $html = "";
       ...
       // Plugins case
        if (
            ...
            $html .= $val['itemtype']::displayPlanningItem($val, $who, $type, $complete, $options); // INFOTEL
        }
